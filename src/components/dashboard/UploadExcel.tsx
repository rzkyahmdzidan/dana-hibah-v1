  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={onLoadSample} className="text-xs text-blue-600 hover:text-blue-800 border border-blue-200 hover:border-blue-400 rounded-lg px-3 py-1.5 transition-colors font-medium">
          Gunakan Data Contoh
        </button>
      </div>
      ...
      {uploadStatus && <div className="mt-3 bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-2 text-sm text-emerald-700">✓ {uploadStatus}</div>}

      {error && <div className="mt-3 bg-rose-50 border border-rose-200 rounded-lg px-4 py-2 text-sm text-rose-600">{error}</div>}

      <details className="mt-4">
        <summary className="text-xs text-slate-400 cursor-pointer hover:text-slate-600 select-none">Lihat format kolom yang didukung ▾</summary>
        <div className="mt-2 text-xs text-slate-500 grid grid-cols-2 gap-x-4 gap-y-1 bg-slate-50 rounded-lg p-3">
          {["No", "Tipe Hibah", "No SPHL", "Tanggal SPHL", "Kode KPPN", "BA Eselon I", "Kode Satker", "Nama Satker", "Lokasi Satker", "No Register", "Mata Uang", "Nilai Belanja", "Nilai Pendapatan"].map((col) => (
            <span key={col} className="font-mono bg-white border border-slate-200 rounded px-1.5 py-0.5">
              {col}
            </span>
          ))}
        </div>
